...
rclmat_w <- matrix(rcl_d, ncol=2, byrow=TRUE)
basemap_w<- classify(basemap,rclmat_w)
saveRDS(basemap_w, "WetNature.rds")
#basemap_w2 <- terra::project(basemap_w, "+proj=utm +zone=32 +ellps=intl +units=m +no_defs", method = "near")
...
AllPlots <- st_join(Novana_plots,Biow_plots, suffix=c("",".y")) %>%
  ...
  vect() %>%
  terra::project(terra::crs(basemap_w))



Green <-  basemap_w %>% terra::extract(AllPlots) %>% pull(C_01)
Green <- !is.na(Green)

WetNaturePlot <- AllPlots[Green,]

WetNaturePlot <-  WetNaturePlot[WetNaturePlot$Habitat %in% c("Ferskeng",
                                                             "Mose og Kær", "Rigkær",
                                                             "Tidvis våd eng",
                                                             "Kalkrige moser",
                                                             "Næringsfattige, sure moser"),]


WetNaturePlot %>% st_as_sf() %>% saveRDS("WetNaturePlot.rds")

#saveRDS(WetNaturePlot, "WetNaturePlot.rds")
...
for(x in 1:1000){
  if(x == 1){
    ...
    P <- basemap_w %>% terra::crop(Buffer) %>% terra::mask(Buffer)%>%  patches()
    ...
    Intersected <- terra::intersect(WetNaturePlot, Buffer) %>%
      as.data.frame() %>%
      pull(AktID)
...
saveRDS(SuperMax, "SuperMaxWet.rds")
...
saveRDS(SuperMaxCoords, "SuperMaxCoordsWet.rds")
...
saveRDS(Summary, "SummaryWet.rds")
...
Summary %>% dplyr::filter(habitat %in% MoreThanFive$habitat) %>%
  ...
  geom_boxplot() +
  theme_bw() +
  labs(x = "Habitat", y = "Average proportion of nature wet")
...
saveRDS(Candidates, "CandidatesWet.rds")
